...
void eval_micro_sequencer(void);
void cycle_memory(void);
void eval_bus_drivers(void);
void drive_bus(void);
...
typedef struct System_Latches_Struct {
	...
	int READY;	/* ready bit */
	/*
	 * The ready bit is also latched as you dont want the memory system
	 * to assert it at a bad point in the cycle
	 */
	...
	/*
	 * MODIFY:
	 * you should add here any other registers you need to
...
void load_program(char *program_filename, int is_virtual_base)
{
	...
	if (is_virtual_base) {
		...
		pte = (MEMORY[(CURRENT_LATCHES.PTBR + (((program_base << 1) >> PAGE_NUM_BITS) << 1)) >> 1][1]
				<< 8) |
