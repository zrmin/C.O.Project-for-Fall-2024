...
void init_control_store(char *ucode_filename)
{
        ...
        for(i = 0; i < CONTROL_STORE_ROWS; i++) {
                ...
                if (line[index] != '\0')
                        printf("Warning: Extra bit(s) in control store file \
                                        %s. Line: %d\n", ucode_filename, i);
        }
        printf("\n");
}
...
int main(int argc, char *argv[])
{
        ...
        /* Error Checking */
        if (argc < 3) {
                printf("Error: usage: %s <micro_code_file> \
...
void DE_stage() {
        ...
        if (LD_AGEX) {
                 ...
                 *
                 */
                for (ii = COPY_AGEX_CS_START; ii< NUM_CONTROL_STORE_BITS; \
                                                                ii++) {
                        NEW_PS.AGEX_CS[jj++] = \
                                  CONTROL_STORE[CONTROL_STORE_ADDRESS][ii];
                }
        }
